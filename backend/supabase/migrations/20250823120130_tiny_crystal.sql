...
CREATE TABLE IF NOT EXISTS notifications (
    ...
    type ENUM('low_stock', 'sale_alert', 'system', 'promotion') NOT NULL,
...
-- =====================================================
-- 10. AUDIT LOG TABLE (Security & Tracking)
-- =====================================================
...
-- Insert Ethiopian Product Categories
...
BEGIN
    ...
    DECLARE v_profit_amount DECIMAL(10,2);
    ...
    INTO v_cost_price, v_current_quantity
    FROM inventory_items 
    ...
    -- Calculate amounts
    SET v_total_amount = p_unit_price * p_quantity;
    SET v_profit_amount = (p_unit_price - v_cost_price) * p_quantity;
    
    -- Insert sale record
    INSERT INTO sales (
        uuid, user_id, item_id, quantity, unit_price, cost_price, 
        total_amount, profit_amount, payment_method, customer_name, 
        customer_phone, notes
    ) VALUES (
        UUID(), p_user_id, p_item_id, p_quantity, p_unit_price, v_cost_price,
        v_total_amount, v_profit_amount, p_payment_method, p_customer_name,
        p_customer_phone, p_notes
    );
    ...
    INSERT INTO stock_movements (
        uuid, user_id, item_id, movement_type, quantity_change,
        previous_quantity, new_quantity, reference_id, notes
    ) VALUES (
        UUID(), p_user_id, p_item_id, 'sale', -p_quantity,
        v_current_quantity, v_current_quantity - p_quantity,
        v_sale_id, CONCAT('Sale #', v_sale_id)
    );
    
    COMMIT;
    
    SELECT v_sale_id as sale_id, v_total_amount as total_amount, v_profit_amount as profit_amount;
END //
...
BEGIN
    IF NEW.quantity <= NEW.min_threshold AND OLD.quantity > OLD.min_threshold THEN
        INSERT INTO notifications (uuid, user_id, type, title, message)
        VALUES (
            UUID(),
            NEW.user_id,
            'low_stock',
            'Low Stock Alert',
            CONCAT('Item "', NEW.name, '" is running low. Current stock: ', NEW.quantity, ', Minimum threshold: ', NEW.min_threshold)
        );
    END IF;
END //
...
-- =====================================================
-- GRANT PERMISSIONS (Run as MySQL root user)
-- =====================================================

-- Create application user (optional, for security)
-- CREATE USER 'gebeyanet_user'@'localhost' IDENTIFIED BY 'secure_password_here';
-- GRANT SELECT, INSERT, UPDATE, DELETE ON gebeyanet_db.* TO 'gebeyanet_user'@'localhost';
-- FLUSH PRIVILEGES;
